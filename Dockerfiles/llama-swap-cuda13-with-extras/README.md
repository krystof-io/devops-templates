...
| `llama-server` | ggml-org/llama.cpp b9567 | LLM inference (CUDA 13.2) |
...
docker build --progress=plain \
  ...
  -t <repohost>/krystof-io/llama-swap-cuda13-with-extras:latest \
  .
```
...
```bash
docker push <repohost>/krystof-io/llama-swap-cuda13-with-extras:latest
```
...
- Component versions are pinned via build-args at the top of the `Dockerfile`
  (`LLAMA_CPP_VERSION`, `LLAMA_SWAP_VERSION`, `WHISPER_CPP_VERSION`,
  `SD_CPP_VERSION`) — bump them there to upgrade.
